...
const NewClass: React.FC = () => {
  ...
  const [scheduleItems, setScheduleItems] = useState([INITIAL_SCHEDULE_ITEM]);

  const addNewScheduleItem = () => {
    setScheduleItems([...scheduleItems, INITIAL_SCHEDULE_ITEM]);
  };
  ...
  const handleCreateClass = async (e: FormEvent) => {
    ...
    try {
      await api.put(`/users/${user?.id}/edit`, { whatsapp: whatsapp });
          ...
          <fieldset>
            ...
            <InputGroup>
              <Me>
                ...
                <strong>{`${UserName ? UserName[0] : user.name}`}</strong>
              </Me>
              <Input
                flex={4}
                name="whatsapp"
