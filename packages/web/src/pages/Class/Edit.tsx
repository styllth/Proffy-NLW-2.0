...
const EditClass: React.FC<RouterProps> = props => {
  ...
  const ChangeClassStatus = (field: Field, value: number | string) => {
    if (Class) {
  ...
  const ChangeVideoData = (field: Video, value: number | string | File) => {
        ...
        <Main onSubmit={updateClassData}>
          ...
          <fieldset className="videoZone">
            <legend>
              Videos gratuitos
              {isTheUserLoggedIn && (
                <button type="button" onClick={() => setAddVideo(true)}>
                  + Novo Video
                </button>
              )}
            </legend>
            ...
            {addVideo && (
              <>
                ...
                <button type="button" onClick={videoUpload}>
                  Enviar video
                </button>
              </>
            )}
            {!addVideo && (
              <button
                type="button"
                onClick={() => history.push(`/to_watch/${class_id}`)}
              >
                Assistir aos videos
              </button>
            )}
