  ...
  const handleSelectSubject = (e: OptionsType) => {
    if (classes) {
      const [selected] = classes.filter(
        (props): boolean => props.subject === e.value
      );
      ...
      <Main onSubmit={updateUserData}>
        ...
        <fieldset>
          ...
          {selectedClass?.class_schedule ? (
            selectedClass.class_schedule.map(
              (item: ClassSchedule, index: number) => (
          ...
          {selectedClass ? (
            <p>
              <Link to={`/class/${selectedClass.id}`}>Abrir Materia</Link>
            </p>
          ) : (
            <p>
              <Link to="/class/new">Criar Materia</Link>
            </p>
          )}
        </fieldset>
      </Main>
